...
DB=os.getenv('RH_DB_PATH','riemann_lab.sqlite3'); LOCK=threading.RLock()
...
INTERVAL=max(0.0,float(os.getenv('RH_SEARCH_INTERVAL','0.05'))); OFFLINE_EVERY=max(1,int(os.getenv('RH_OFFLINE_SCAN_INTERVAL','25')))
...
def addcandidate(source,s,res,dev,e):
 with LOCK:
  c=db();cur=c.execute('INSERT INTO candidates VALUES(NULL,?,?,?,?,?,?,?,?,?)',(source,mp.nstr(s.real,VERIFY_DPS),mp.nstr(s.imag,VERIFY_DPS),mp.nstr(res,60),mp.nstr(dev,60),VERIFY_DPS,'numerically_survived',json.dumps(e),now()));c.commit();i=cur.lastrowid;c.close();return i
...
def verify(s):
 with mp.workdps(VERIFY_DPS):
  x=mp.mpc(s)
  for _ in range(8):
   f=mp.zeta(x);df=mp.diff(mp.zeta,x)
 ...
 with mp.workdps(VERIFY_DPS):
  for r in ['.10','.20','.30','.40','.45','.55','.60','.70','.80','.90']:
   for d in ['-.35','-.15','0','.15','.35']:
    ...
    try:
     root=mp.findroot(mp.zeta,(seed,seed+mp.mpc('.003','.003')),solver='secant',tol=mp.mpf('1e-70'),maxsteps=60,verify=False);res=abs(mp.zeta(root));dev=abs(mp.re(root)-mp.mpf('.5'))
...
def ai_report(e):
 ...
 c=Client(host='https://ollama.com',headers={'Authorization':f'Bearer {API_KEY}'})
 sys='You are a mathematical research-report assistant. Explain why the program stopped, preserve numerical evidence, distinguish numerical evidence from formal proof, and never claim the Riemann Hypothesis is disproved unless the supplied evidence actually establishes that. Do not invent facts or computations.'
 r=c.chat(model=MODEL,messages=[{'role':'system','content':sys},{'role':'user','content':'Generate a cautious research report from this evidence:\n'+json.dumps(e,indent=2)}]);return r['message']['content']
async def checkpoint(paused=False,extra=None):
 d={'next_n':int(state('next_n','1')),'zeros_checked':count('zeros'),'updated_at_utc':now(),'paused':paused}
 if extra:d.update(extra)
 try:return await save_checkpoint(d)
 except Exception as ex:search.error=f'Checkpoint save failed: {type(ex).__name__}: {ex}';return False
class Search:
 def __init__(self):self.running=False;self.stopped=False;self.reason='';self.error='';self.last=None;self.task=None
 def status(self):return {'running':self.running,'stopped':self.stopped,'reason':self.reason,'error':self.error,'next_n':int(state('next_n','1')),'zeros_checked':count('zeros'),'candidate_count':count('candidates'),'search_dps':SEARCH_DPS,'verify_dps':VERIFY_DPS,'checkpoint':'github' if os.getenv('CHECKPOINT_TOKEN') else 'local-only','last_zero':self.last,'candidate':latest('candidates'),'report':latest('reports')}
 async def start(self):
  if self.running:return
  cp=await load_checkpoint()
  if cp and int(cp.get('next_n',1))>=1:setstate('next_n',int(cp['next_n']))
 ...
 async def stop(self):
  self.running=False;self.stopped=False;self.reason='Manually stopped';await checkpoint(True)
 async def candidate(self,source,s,res,dev,extra):
  ...
  cid=addcandidate(source,s,res,dev,e);self.running=False;self.stopped=True;self.reason='A numerically verified nontrivial zero candidate has Re(s) != 1/2';await checkpoint(True,{'candidate':e,'candidate_found':True})
 ...
 async def run(self):
  try:
   while self.running:
    ...
    for n in range(start,start+BATCH):
     ...
     if MAX_N and n>MAX_N:self.running=False;self.reason=f'Configured maximum n={MAX_N} reached';await checkpoint(True);return
  ...
  except Exception as ex:self.running=False;self.reason='Search paused because of an execution error';self.error=repr(ex);await checkpoint(True,{'error':repr(ex)})
