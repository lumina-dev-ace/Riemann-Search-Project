...
def ai_report(e):
 ...
 r=client.chat(model=MODEL,messages=[{'role':'system','content':sys},{'role':'user','content':'Generate a cautious research report from this evidence:\n'+json.dumps(e,indent=2)}])
 return r['message']['content']

class Checkpoint(BaseModel): next_n:int|None=None
class Search:
 ...
 async def start(self,checkpoint=None):
  ...
  if checkpoint is not None and checkpoint>=1:setstate('next_n',checkpoint)
  ...
  try:
   while self.running and not self.stopped:
    ...
    for n in range(start,start+BATCH):
     ...
     if MAX_N and n>MAX_N:self.running=False; self.stopped=False; self.reason=f'Configured maximum n={MAX_N} reached.'; return
...
@app.post('/api/start')
async def start(cp:Checkpoint|None=None):await search.start(cp.next_n if cp else None);return search.status()
@app.post('/api/stop')
...
HTML='''<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Riemann Search Lab</title><style>body{font-family:system-ui;background:#0d1117;color:#e6edf3;max-width:1100px;margin:auto;padding:25px}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px}.card{background:#161b22;border:1px solid #30363d;border-radius:12px;padding:15px;margin:12px 0}.v{font-size:24px;font-weight:700}button{padding:10px 16px;margin:5px}pre{white-space:pre-wrap;overflow:auto}table{width:100%}</style></head><body><h1>🧮 Riemann Hypothesis Search Lab</h1><p>Continuous numerical search with high-precision verification.</p><button onclick="start()">Start / Resume</button><button onclick="stop()">Stop & Save</button><div class="grid"><div class="card">Status<div id="s" class="v">—</div></div><div class="card">Next n<div id="n" class="v">—</div></div><div class="card">Zeros checked<div id="c" class="v">—</div></div><div class="card">Verification digits<div id="d" class="v">—</div></div></div><div class="card"><h2>Latest zero</h2><pre id="z">—</pre></div><div class="card"><h2>Candidate</h2><pre id="ca">None</pre></div><div class="card"><h2>AI report</h2><pre id="r">None</pre></div><script>
const KEY='riemann-search-checkpoint-v2';
function save(x){if(x&&x.next_n>=1)localStorage.setItem(KEY,String(x.next_n));}
function load(){let x=parseInt(localStorage.getItem(KEY)||'1',10);return Number.isFinite(x)&&x>=1?x:1;}
async function start(){let cp=load();let x=await(await fetch('/api/start',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({next_n:cp})})).json();save(x);f();}
async function stop(){let x=await(await fetch('/api/stop',{method:'POST'})).json();save(x);f();}
async function f(){let x=await(await fetch('/api/status')).json();save(x);s.textContent=x.running?'RUNNING':x.stopped?'CANDIDATE FOUND':'PAUSED';n.textContent=x.next_n;c.textContent=x.zeros_checked;d.textContent=x.verify_dps+' digits';z.textContent=JSON.stringify(x.last_zero,null,2);ca.textContent=x.candidate?JSON.stringify(x.candidate,null,2):'None';r.textContent=x.report?x.report.report:'None';}
