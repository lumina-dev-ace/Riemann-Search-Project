...
def addzero(n,s,sr,vr,dev,verified,suspicious):
 with LOCK:
  c=db();c.execute('INSERT INTO zeros VALUES(NULL,?,?,?,?,?,?,?,?,?,?,?)',(n,mp.nstr(s.real,VERIFY_DPS),mp.nstr(s.imag,VERIFY_DPS),mp.nstr(sr,40),mp.nstr(vr,40),SEARCH_DPS,VERIFY_DPS,int(verified),int(suspicious),'mpmath zetazero + high-precision verification',now()));c.commit();c.close()
...
def compute_zero(n):
 with mp.workdps(SEARCH_DPS):
  s=mp.zetazero(n);sr=abs(mp.zeta(s))
 vr,x,dev=verify(s)
 return s,x,sr,vr,dev
...
def ai_report(e):
 ...
 system=('You are a mathematical research-report assistant. Explain why the program stopped, preserve numerical evidence, distinguish numerical evidence from formal proof, and never claim the Riemann Hypothesis is disproved unless the supplied evidence actually establishes that. Do not invent facts or computations.')
...
class Search:
 ...
 async def candidate(self,source,s,res,dev,extra):
  ...
  cid=addcandidate(source,s,res,dev,e);self.running=False;self.stopped=True;self.reason='A numerically verified nontrivial zero candidate has Re(s) != 1/2';setstate('phase','candidate_found');await checkpoint(True,{'candidate':e,'candidate_found':True})
  ...
  try:
   while self.running:
    ...
    for n in range(start,start+BATCH):
     ...
     setstate('current_n',n);setstate('phase','locating_and_verifying')
     s,root,sr,vr,dev=await asyncio.to_thread(compute_zero,n)
     verified=mp.im(root)>10 and 0<mp.re(root)<1 and vr<mp.mpf('1e-50');suspicious=dev>FINAL
     addzero(n,root,sr,vr,dev,verified,suspicious);setstate('next_n',n+1);setstate('phase','zero_completed')
     ...
     if n%OFFLINE_EVERY==0:
      setstate('phase','off_critical_exploration');x=await asyncio.to_thread(exploratory,root.imag)
      if x is not None:
       vr2,r2,d2=verify(x)
