...
MODEL=os.getenv('OPENAI_MODEL','gpt-5-mini'); API_KEY=os.getenv('OPENAI_API_KEY','')
...
def ai_report(e):
    if not API_KEY: return 'OPENAI_API_KEY is not set. Numerical evidence follows:\n\n'+json.dumps(e,indent=2)
    ...
    r=client.responses.create(model=MODEL,instructions=instructions,input='Generate a cautious research report from this evidence:\n'+json.dumps(e,indent=2))
...
class Search:
    ...
    async def start(self):
        if self.running or self.stopped:return
        self.running=True; self.task=asyncio.create_task(self.run())
    async def stop(self,reason='Manually stopped'):
        self.running=False; self.stopped=True; self.reason=reason
    ...
    async def run(self):
        try:
            while self.running and not self.stopped:
                ...
                for n in range(start,start+BATCH):
                    if MAX_N and n>MAX_N:return await self.stop(f'Configured maximum n={MAX_N} reached.')
        ...
        except Exception as ex:self.running=False; self.stopped=True; self.reason='Search stopped because of an execution error.'; self.error=repr(ex)
...
@app.post('/api/start')
async def start():
    if search.stopped: raise HTTPException(409,'This search run has permanently stopped.')
